...
/** Both renderings of Unix timestamp 0. The endpoint emits the 1969 form; the 1970 form is the same instant. */
const EPOCH_PLACEHOLDER_DATES = new Set(['1969-12-31', '1970-01-01']);
/**
 * The endpoint's other stand-in for a missing incident date. It is spread across at least eight
 * distinct days in 1901 rather than one fixed value, so this matches the year — no vehicle
 * complaint describes a real 1901 incident.
 */
const PLACEHOLDER_YEAR_PREFIX = '1901-';
...
/** ISO YYYY-MM-DD, the only shape the ordering comparisons below are valid on. */
const ISO_DATE_PATTERN = /^\d{4}-\d{2}-\d{2}$/;

/**
 * A model year opens in the calendar year before it — a 2016 model can be on the road in 2015 —
 * so `modelYear - 1` is the earliest year a vehicle of that model year can be involved in
 * anything. Not a tuned floor: it moves with each record.
 */
const MODEL_YEAR_LEAD_YEARS = 1;

/**
 * Resolve `dateOfIncident` against the bounds the complaint record supplies for itself.
 *
 * Placeholders are handled first and stay silent. `12/31/1969`, `01/01/1970`, and any 1901 date
 * are what the endpoint writes where the complainant gave no incident date, so there is nothing
 * to report. Matching those values rather than a date floor is what keeps genuine 1970s and
 * 1980s incidents — a complaint about a 1986 vehicle can carry a 1989 incident date.
 * `dateComplaintFiled` is generated by NHTSA on receipt, never a placeholder, so it skips this.
 *
 * What remains are upstream transcription errors — years like 1016 for 2016, or 2203 for 2023.
 * They are caught with the record's own fields rather than a calendar cutoff, because every
 * fixed cutoff either sits above real dates it then destroys or below typos it then misses:
 *
 *   - an incident cannot happen after the complaint reporting it was filed;
 *   - an incident cannot happen before the vehicle existed.
 *
 * A rejected value is reported rather than dropped, because dropping it would be
 * indistinguishable from a placeholder and the two say different things: a placeholder means
 * NHTSA holds no incident date, a rejected value means NHTSA holds one its own record
 * contradicts. Values that cannot be parsed as dates at all pass through untouched — the
 * comparisons are meaningless on them, and inventing a verdict is worse than forwarding the
 * string NHTSA sent.
 */
...
): Pick<Complaint, 'dateOfIncident' | 'unreliableIncidentDate'> {
  const date = normalizeComplaintDate(value);
  if (!date) return {};
  if (EPOCH_PLACEHOLDER_DATES.has(date) || date.startsWith(PLACEHOLDER_YEAR_PREFIX)) return {};
  if (!ISO_DATE_PATTERN.test(date)) return { dateOfIncident: date };

  if (
    dateComplaintFiled !== undefined &&
    ISO_DATE_PATTERN.test(dateComplaintFiled) &&
    date > dateComplaintFiled
  ) {
    return { unreliableIncidentDate: { reported: date, reason: 'postdates_filing' } };
  }

  if (
    modelYear !== undefined &&
    modelYear !== MODEL_YEAR_NOT_APPLICABLE &&
    Number(date.slice(0, 4)) < modelYear - MODEL_YEAR_LEAD_YEARS
  ) {
    return { unreliableIncidentDate: { reported: date, reason: 'predates_model_year' } };
  }
...
export class NhtsaService {
  ...
  async getComplaintsByVehicle(
    ...
    return (data.results ?? []).map((raw) => normalizeComplaint(raw, modelYear));
  }
...
/**
 * `modelYear` is the year the caller queried, which is also the model year of every complaint
 * the endpoint returns for that query — it is the second bound `resolveIncidentDate` needs.
 */
function normalizeComplaint(r: RawComplaint, modelYear?: number): Complaint {
  const dateComplaintFiled = normalizeComplaintDate(r.dateComplaintFiled);
  return omitUndefined({
    ...
    ...resolveIncidentDate(r.dateOfIncident, dateComplaintFiled, modelYear),
    dateComplaintFiled,
